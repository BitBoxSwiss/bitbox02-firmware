...
VOID USBD_Custom_HID_SetReport(struct UX_SLAVE_CLASS_HID_STRUCT *hid_instance)
{
  ...
  if (ux_device_class_hid_receiver_event_get(hid_instance, &hid_event) == UX_SUCCESS)
  {
    got_report(hid_event.ux_device_class_hid_received_event_data,
               (int)hid_event.ux_device_class_hid_received_event_length);
