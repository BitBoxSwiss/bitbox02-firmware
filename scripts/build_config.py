...
class Config:
    ...
    def profile(self) -> str:
        if self.debug == "yes":
            return "debug"
        return "release" if self.product == "bitbox03" else "relwithdebinfo"
    ...
    def validate(self) -> Config:
        ...
        if self.debug not in ("no", "yes"):
            raise ValueError(f"Invalid debug setting: {self.debug}")
    ...
    def describe(self) -> None:
        ...
        print(f"Debug: {self.debug}; profile: {self.profile}")
...
def add_config_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--debug", choices=("no", "yes"), help="Debug build (default: no)")
...
def interactive(saved: dict[str, str]) -> Config:
    ...
    debug = choose("Debug", ("no", "yes"), saved.get("DEBUG", base.debug))
    ...
    return Config(product, board, edition, software, int(speed), debug).validate()
...
def save_config(path: Path, config: Config) -> None:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            ...
            for key in ("product", "board", "edition", "debug", "probe_software", "swd_speed"):
...
def image_info(
    ...
    if config.product == "bitbox03":
        ...
        elf = build_dir(config, root=root) / CARGO_TARGET / profile / name
    ...
    directory = build_dir(config, profile, root) / "bin"
