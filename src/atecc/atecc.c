...
static ATCA_STATUS _factory_setup(
...
int atecc_setup(
    ...
    const uint8_t* encryption_key)
{
    if (io_protection_key == NULL || auth_key == NULL || encryption_key == NULL) {
        return SC_ERR_IFS;
    }
...
#if FACTORYSETUP == 1
    result = _factory_setup(io_protection_key, auth_key, encryption_key);
    if (result != ATCA_SUCCESS) {
        return result;
    }
#endif
 ...
 * ATECC_SLOT_AUTHKEY.
 */
static ATCA_STATUS _authorize_key(const uint8_t* auth_key)
{
...
bool atecc_gen_attestation_key(const uint8_t* auth_key, uint8_t* pubkey_out)
{
    ATCA_STATUS result = _authorize_key(auth_key);
    if (result != ATCA_SUCCESS) {
        return false;
    }
