    ...
    size_t data_size)
{
    ...
    if (data_size != ATCA_BLOCK_SIZE && data_size != 2 * (size_t)ATCA_BLOCK_SIZE) {
...
void atecc_io_clear_tempkey(void)
{
    util_zero(&_async.io_temp_key, sizeof(_async.io_temp_key));
}
