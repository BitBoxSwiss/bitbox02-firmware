...
/**
 * @brief Verifies that the entire flash memory is erased.
 *
 * This function scans the full memory and checks whether all bytes are 0xFF,
 * indicating an erased state. It uses block-wise reads for efficiency and
 * logs the first mismatch found (if any).
 *
 * @return true if the entire memory is erased (all 0xFF), false otherwise.
 */
USE_RESULT bool spi_mem_verify_erased(void);
