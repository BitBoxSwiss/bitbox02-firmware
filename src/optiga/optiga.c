    ...
    size_t* data_len_out)
{
    ...
    uint8_t metadata_size = metadata[1];
    if ((size_t)(metadata_size + 2) > metadata_len) {
        // Malformed metadata: declared size exceeds buffer length
        return false;
    }

    const uint8_t* tlv = &metadata[2];
    size_t tlv_len = metadata_size;

    size_t offset = 0;

    while (offset + 2 <= tlv_len) { // Ensure at least <tag><size> are available
        uint8_t current_tag = tlv[offset];
        uint8_t size = tlv[offset + 1];

        if (offset + 2 + size > tlv_len) {
        ...
        if (current_tag == tag) {
            ...
            if (data_out) {
                memcpy(data_out, &tlv[offset + 2], size);
            }
            if (data_len_out) {
                *data_len_out = size;
            }
    ...
    size_t check_tags_len)
{
    ...
    uint8_t expected_tag_data[100] = {0};
    size_t expected_tag_data_len = 0;
    uint8_t actual_tag_data[100] = {0};
    size_t actual_tag_data_len = 0;
...
static int _optiga_verify_password(const char* password, uint8_t* password_secret_out)
{
    ...
    if (res != OPTIGA_LIB_SUCCESS || password_secret_size != 32) {
        goto cleanup;
    }
...
static int _verify_config(void)
{
    ...
    /* int pw_res = optiga_init_new_password("password"); */
    /* if (pw_res) { */
    /*     util_log("set_password failed: %d", pw_res); */
    /* } else { */
    /*     util_log("set password OK"); */
    /* } */
    /* res = _optiga_verify_password("password"); */
    /* if (res) { */
    /*     util_log("verify_password failed: %x", (unsigned int)res); */
    /* } */

    uint8_t buf[4] = {0};
    uint16_t size = sizeof(buf);
    res = _optiga_util_read_data_sync(_util, OID_COUNTER_PASSWORD, 0, buf, &size);
    if (res != OPTIGA_LIB_SUCCESS) {
        return res;
    }
    util_log("password ctr: %d %d %d %d", buf[0], buf[1], buf[2], buf[3]);
