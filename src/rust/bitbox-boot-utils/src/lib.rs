...
/// # Safety
///
/// `slot_address` must be aligned for `u32` and point to readable, initialized
/// memory containing an [`ImageHeader`]. If that header describes a vector
/// table within `slot_len`, the first two `u32` entries of the vector table must
/// also be readable and initialized. This memory must not be mutated for the
/// duration of the call.
pub unsafe fn vector_table_from_image_header(
    slot_address: usize,
    slot_len: usize,
    ram_base: usize,
    ram_len: usize,
...
mod tests {
    ...
    const TEST_IMAGE_LEN: usize = IMAGE_HEADER_LEN + 8;
    ...
    fn build_test_image<const N: usize>(
        ...
        assert!(N >= core::mem::size_of::<ImageHeader>());
        let mut image = TestImage([0u8; N]);
        let header = build_test_image_header(magic, header_len, code_size);
        image.0[..core::mem::size_of::<ImageHeader>()].copy_from_slice(&header);
        image
    }
    ...
    fn assert_test_image_invalid(magic: [u8; 4], header_len: u32, code_size: u32, slot_len: usize) {
        ...
        assert_eq!(
            // SAFETY: TestImage is u32-aligned and remains readable and
            // immutable for the call. It contains the vector-table entries
            // whenever the header passes the preceding validation.
            unsafe {
    ...
    fn test_image_header_try_from_bytes_invalid_len() {
        assert_eq!(
            ImageHeader::try_from_bytes(&[0; core::mem::size_of::<ImageHeader>() - 1]),
            Err(())
        );
        assert_eq!(
            ImageHeader::try_from_bytes(&[0; core::mem::size_of::<ImageHeader>() + 1]),
    ...
    fn test_vector_table_from_image_header_invalid_magic() {
        assert_test_image_invalid(*b"NOPE", IMAGE_HEADER_LEN as u32, 8, TEST_IMAGE_LEN);
    }
    ...
    fn test_vector_table_from_image_header_ram_range_overflow() {
        ...
        assert_eq!(
            // SAFETY: TestImage is u32-aligned and remains readable and
            // immutable for the call, including both vector-table entries.
            unsafe {
                vector_table_from_image_header(
                    image.0.as_ptr() as usize,
                    image.0.len(),
