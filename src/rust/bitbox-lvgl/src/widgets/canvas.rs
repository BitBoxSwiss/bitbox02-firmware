...
pub trait CanvasExt: ImageExt {
    ...
    unsafe fn set_buffer_raw(
        ...
        unsafe { ffi::lv_canvas_set_buffer(self.as_ptr(), buf, width, height, color_format) }
    }

    /// # Safety
    /// `draw_buf` must remain valid for the lifetime required by LVGL.
    unsafe fn set_draw_buf_raw(&self, draw_buf: *mut ffi::lv_draw_buf_t) {
...
impl LvHandle<class::CanvasTag> {
    pub fn new(
        ...
        let attachment = util::attach_to_object(canvas.as_ptr(), data)
