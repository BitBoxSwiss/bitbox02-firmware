...
pub trait ImageExt: ObjExt {
    ...
    /// # Safety
    /// `src` must remain valid for the lifetime required by LVGL.
    unsafe fn set_src_raw(&self, src: *const c_void) {
        unsafe { ffi::lv_image_set_src(self.as_ptr(), src) }
    }
