...
impl Error {
    fn from_hal_status(status: bitbox_platform_stm32u5_sys::HAL_StatusTypeDef) -> Result<(), Self> {
        match status as u32 {
            0 => Ok(()),
...
impl Uart {
    ...
    fn write_chunk(&mut self, chunk: &[u8]) -> Result<(), Error> {
        let status = unsafe {
            bitbox_platform_stm32u5_sys::HAL_UART_Transmit(
                &raw mut crate::ffi::huart1 as *mut _,
                chunk.as_ptr(),
                chunk.len() as u16,
                self.timeout_ms,
            )
