...
async fn authorize_key<T: Timer>(memory: &mut impl Memory) -> Result<(), i32> {
    ...
    let response = ops::host_check_mac(&num_in, &rand_out, auth_key.as_ref())?;
...
async fn update_kdf_key<T: Timer>(
    ...
    let rand_out = ops::chip_nonce_rand::<T>(&num_in).await?;
    ops::host_nonce(&num_in, &rand_out)?;
    ops::chip_gendig_encryption_key::<T>().await?;
    let encryption_key = load_encryption_key(memory);
    ops::host_gendig(encryption_key.as_ref())?;

    let (cipher, mac) = ops::host_write_auth_mac(SLOT_KDF, 0, new_key.as_ref())?;
...
async fn write_data_slot_block<T: Timer>(
    ...
    let (cipher, mac) = ops::host_write_auth_mac(slot, block, bytes)?;
...
mod tests {
    ...
    fn test_drop_clears_io_temp_key() {
        ...
        let mut task: util::bb02_async::Task<_> = alloc::boxed::Box::pin(async {
            ...
            let num_in = Zeroizing::new([0u8; ops::NONCE_NUMIN_SIZE]);
            let rand_out = Zeroizing::new([0u8; 32]);
            ops::host_nonce(&num_in, &rand_out).unwrap();
