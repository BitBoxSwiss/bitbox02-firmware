...
fn hmac_sha256(key: &[u8], data: &[u8], out: &mut [u8; KDF_LEN]) {
    let mut mac = HmacSha256::new_from_slice(key).unwrap();
    mac.update(data);
    FixedOutput::finalize_into(mac, out.into());
}

fn hmac_sha256_overwrite(key: &[u8], out: &mut [u8; KDF_LEN]) {
    let mut mac = HmacSha256::new_from_slice(key).unwrap();
    mac.update(out);
    FixedOutput::finalize_into(mac, out.into());
}

fn sha256(data: &[u8], out: &mut [u8; KDF_LEN]) {
    let mut hasher = Sha256::new();
    hasher.update(data);
    FixedOutput::finalize_into(hasher, out.into());
}
