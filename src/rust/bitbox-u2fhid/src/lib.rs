...
mod tests {
    ...
    #[test]
    fn test_ping_single_packet() {
        let mut transport = make_transport();
        transport.handle_report(&request_reports(TEST_CID, COMMAND_PING, b"hello")[0], 0);

        let reports = drain_reports(&mut transport);
        let (cid, cmd, payload) = parse_message(&reports);
        assert_eq!(cid, TEST_CID);
        assert_eq!(cmd, COMMAND_PING);
    ...
    fn test_unused_bytes_are_zeroed() {
        let mut transport = make_transport();
        transport.handle_report(&request_reports(TEST_CID, COMMAND_PING, b"x")[0], 0);
    ...
    #[test]
    fn test_init_broadcast_allocates_cid() {
        let mut transport = make_transport();
        transport.handle_report(
            &request_reports(BROADCAST_CID, COMMAND_INIT, b"12345678")[0],
            0,
        );

        let (cid, cmd, payload) = parse_message(&drain_reports(&mut transport));
        assert_eq!(cid, BROADCAST_CID);
        assert_eq!(cmd, COMMAND_INIT);
        assert_eq!(&payload[..8], b"12345678");
        let allocated_cid = u32::from_be_bytes(payload[8..12].try_into().unwrap());
        assert_ne!(allocated_cid, 0);
        assert_ne!(allocated_cid, BROADCAST_CID);
        assert_eq!(payload[12], 2);
        assert_eq!(payload[16], CAPABILITY_WINK);
    }

    #[test]
    fn test_init_non_broadcast_echoes_cid() {
        let mut transport = make_transport();
        transport.handle_report(&request_reports(TEST_CID, COMMAND_INIT, b"12345678")[0], 0);
    ...
    #[test]
    fn test_wink_zero_length() {
        let mut transport = make_transport();
        transport.handle_report(&request_reports(TEST_CID, COMMAND_WINK, b"")[0], 0);

        let (_, cmd, payload) = parse_message(&drain_reports(&mut transport));
        assert_eq!(cmd, COMMAND_WINK);
        assert!(payload.is_empty());
    }

    #[test]
    fn test_lock_unsupported_returns_invalid_cmd() {
        let mut transport = make_transport();
        transport.handle_report(&request_reports(TEST_CID, COMMAND_LOCK, &[1])[0], 0);

        let (_, cmd, payload) = parse_message(&drain_reports(&mut transport));
        assert_eq!(cmd, COMMAND_ERROR);
        assert_eq!(payload, vec![ErrorCode::InvalidCmd as u8]);
    }

    #[test]
    fn test_max_size_ping() {
        let mut transport = make_transport();
        let payload = vec![0x5a; MAX_MESSAGE_SIZE];
        for report in request_reports(TEST_CID, COMMAND_PING, &payload) {
            transport.handle_report(&report, 0);
        }
