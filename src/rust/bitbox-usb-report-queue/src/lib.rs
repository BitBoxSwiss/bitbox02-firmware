...
impl UsbReportQueue {
    ...
    fn ensure_capacity(&mut self) -> bool {
        if self.reports.capacity() >= USB_REPORT_QUEUE_MAX_LEN {
            return true;
        }

        self.reports
            .try_reserve_exact(USB_REPORT_QUEUE_MAX_LEN - self.reports.capacity())
            .is_ok()
    }

    fn zeroize_contents(&mut self) {
        for report in self.reports.iter_mut() {
            *report = [0; USB_REPORT_SIZE];
        }
    }
    ...
    pub fn clear(&mut self) {
        ...
        self.reports.clear();
        let _ = self.ensure_capacity();
    }

    pub fn push(&mut self, report: &[u8; USB_REPORT_SIZE]) -> UsbReportQueueError {
        if !self.ensure_capacity() || self.reports.len() >= USB_REPORT_QUEUE_MAX_LEN {
    ...
    pub fn pull(&mut self) -> Option<[u8; USB_REPORT_SIZE]> {
        let report = self.reports.front().copied()?;
        *self.reports.front_mut().unwrap() = [0; USB_REPORT_SIZE];
