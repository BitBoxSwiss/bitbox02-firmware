...
async fn process_address(request: &pb::EthPubRequest) -> Result<Response, Error> {
    let params = bitbox02::app_eth::params_get(request.coin as _).ok_or(Error {
    ...
    let erc20_params: Option<bitbox02::app_eth::ERC20Params> =
        if request.contract_address.is_empty() {
            ...
            Some(
                bitbox02::app_eth::erc20_params_get(request.coin as _, address).ok_or(Error {
                    msg: Some("invalid coin".into()),
                    kind: ErrorKind::InvalidInput,
                })?,
            )
    ...
    let pubkey = bitbox02::keystore::secp256k1_pubkey_uncompressed(&request.keypath)
        .map_err(Error::err_invalid_input)
        .context("secp256k1_pubkey_uncompressed failed")?;
...
fn process_xpub(request: &pb::EthPubRequest) -> Result<Response, Error> {
    ...
    bitbox02::app_eth::params_get(request.coin as _).ok_or(Error {
        msg: Some("invalid coin".into()),
        kind: ErrorKind::InvalidInput,
    })?;
...
pub async fn process(request: &pb::EthPubRequest) -> Result<Response, Error> {
    let output_type = OutputType::from_i32(request.output_type).ok_or(Error {
        msg: Some("invalid output_type".into()),
        kind: ErrorKind::InvalidInput,
    })?;
    match output_type {
