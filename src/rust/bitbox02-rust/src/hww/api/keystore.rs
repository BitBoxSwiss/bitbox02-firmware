...
async fn get_mnemonic_passphrase(
    ...
    if supports_host_passphrase {
        ...
        if choice == TrinaryChoice::TRINARY_CHOICE_RIGHT {
            // While we are waiting for the passphrsae from the host, he BitBox shows the regular
            // waiting screen. We could show a custom "Waiting on host..." component, but the
            // problem is that if the host wallet is closed and we never get a response, this screen
            // would remain there and not disappear until a new BitBox connection is made. The
            // reason for this is that while we wait, there is no active request from the host, so
            // there is no polling/pinging, so the regular USB request timeout cancellation which
            // drops the current task never happens.
            return get_mnemonic_passphrase_from_host().await;
        }
    }
    Ok(unlock::enter_mnemonic_passphrase_on_device().await?)
}
