...
pub async fn verify_recipient(recipient: &str, amount: &str) -> Result<(), UserAbort> {
    let future = bitbox02::ui::confirm_transaction_address_create(amount, recipient);
    if screensaver_without(future).await {
        Ok(())
    } else {
        Err(UserAbort)
    }
...
pub async fn verify_total_fee(total: &str, fee: &str, longtouch: bool) -> Result<(), UserAbort> {
    let future = bitbox02::ui::confirm_transaction_fee_create(total, fee, longtouch);
    if screensaver_without(future).await {
        Ok(())
    } else {
        Err(UserAbort)
