...
pub(super) unsafe fn setup(
    ...
    unsafe { bitbox_securechip_sys::atecc_setup(ifs) }
}

pub(super) unsafe fn gen_attestation_key(pubkey_out: *mut u8) -> bool {
    unsafe { bitbox_securechip_sys::atecc_gen_attestation_key(pubkey_out) }
}

pub(super) unsafe fn random(rand_out: *mut u8) -> bool {
    unsafe { bitbox_securechip_sys::atecc_random(rand_out) }
}

pub(super) fn u2f_counter_set_raw(counter: u32) -> bool {
    unsafe { bitbox_securechip_sys::atecc_u2f_counter_set(counter) }
}

#[cfg(feature = "app-u2f")]
pub(super) unsafe fn u2f_counter_inc(counter: *mut u32) -> bool {
    unsafe { bitbox_securechip_sys::atecc_u2f_counter_inc(counter) }
}
