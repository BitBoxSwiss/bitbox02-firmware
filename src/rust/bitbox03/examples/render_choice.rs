...
//! Headless renderer for the trinary choice screen (e.g. 12/24 word count), for visual review
//! through LVGL's real software renderer (no GPU / windowing needed). Renders on the real
//! 480×800 geometry.
//!
//! ```sh
//! cargo run -p bitbox03 --example render_choice -- /tmp/choice.bmp
//! cargo run -p bitbox03 --example render_choice -- /tmp/choice.bmp pressed  # "12" held down
//! sips -s format png /tmp/choice.bmp --out /tmp/choice.png   # macOS; or ImageMagick `convert`
//! ```
...
use bitbox03::ui::choice::build_trinary_choice_screen;
...
fn main() {
    let out_path = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "choice_preview.bmp".to_string());
    ...
    let screen =
        build_trinary_choice_screen("How many words?", Some("12"), None, Some("24"), responder);
    if pressed {
        // Force the pressed look on the first ("12") button. Real input propagates the state to
        // the label via press events; mirror that manually here.
        let actions = screen.child(1).expect("actions row");
        let button = actions.child(0).expect("first choice button");
        button.add_state(LvState::LV_STATE_PRESSED);
        let label = button.child(0).expect("choice label");
    ...
    // Keep pumping briefly so style transitions (e.g. the pressed-state fill) settle and their
    // redraws land in the framebuffer.
    for _ in 0..100 {
        lvgl::timer::handler();
        std::thread::sleep(Duration::from_millis(2));
    }
