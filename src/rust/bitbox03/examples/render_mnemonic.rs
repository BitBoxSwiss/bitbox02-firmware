...
//! Headless renderer for the recovery-words review screen, for visual review through LVGL's real
//! software renderer (no GPU / windowing needed). Renders on the real 480×800 geometry.
//!
//! ```sh
//! cargo run -p bitbox03 --example render_mnemonic -- /tmp/mnemonic.bmp        # 24 words
//! cargo run -p bitbox03 --example render_mnemonic -- /tmp/mnemonic.bmp 12    # 12 words
//! cargo run -p bitbox03 --example render_mnemonic -- /tmp/mnemonic.bmp worst # widest words
//! sips -s format png /tmp/mnemonic.bmp --out /tmp/mnemonic.png   # macOS; or ImageMagick
//! ```
...
use bitbox03::ui::recovery_words::build_recovery_words_screen;
use util::futures::completion::completion;
...
/// The 24 widest BIP39 English words in 32px bold Inter — layout stress test: no word may wrap.
const WORDS_WORST: [&str; 24] = [
    "mushroom", "remember", "document", "december", "hedgehog", "announce", "champion", "cinnamon",
    "maximum", "tomorrow", "exchange", "mechanic", "awesome", "cupboard", "mountain", "symptom",
    "kangaroo", "empower", "mosquito", "purchase", "company", "minimum", "resemble", "response",
];
...
fn main() {
    let out_path = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "mnemonic_preview.bmp".to_string());
    let mode = std::env::args().nth(2).unwrap_or_else(|| "24".to_string());
    let words: Vec<&str> = match mode.as_str() {
        "12" => WORDS_24[..12].to_vec(),
        "worst" => WORDS_WORST.to_vec(),
        _ => WORDS_24.to_vec(),
    };
    ...
    let (responder, _result) = completion();
