fn main() {
    macro_rules! check_versions {
        ($input:expr, $($version:literal),+) => {
            {
                let minor = $input;
                $(
                    if minor >= $version {
                        println!(concat!("cargo:rustc-cfg=rust_v_1_", stringify!($version)));
                    }
                )+
            }
        }
    ...
    if !stdout.starts_with(version_prefix) {
        panic!("unexpected rustc outptu: {}", stdout);
    }
