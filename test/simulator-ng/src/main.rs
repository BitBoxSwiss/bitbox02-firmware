...
// Only send `Copy` things in SDL events. it doesn't handle objects very safely.
// https://github.com/Rust-SDL2/rust-sdl2/issues/487
...
static FONT: &[u8; 10304] = include_bytes!("../../../src/ui/fonts/monogram.ttf");
...
pub fn network_task(
    new_connection_flag: Arc<AtomicBool>,
    evs: mpsc::Receiver<(EventSender, mpsc::Receiver<[u8; 64]>)>,
    hww_sender: mpsc::Sender<[u8; 64]>,
) {
    ...
    for stream in listener.incoming() {
        if counter.fetch_add(2, Ordering::SeqCst) == 0 {
            new_connection_flag.store(true, Ordering::SeqCst);
            let stream = stream.unwrap();
            let (es, receiver) = evs.recv().unwrap();
            thread::spawn({
                let stream = stream.try_clone().unwrap();
                let counter = counter.clone();
                let hww_sender = hww_sender.clone();
                move || handle_stream_reader(stream, hww_sender, es, counter)
            });
            thread::spawn({
                let counter = counter.clone();
...
pub fn handle_mouse_motion(sliders: &mut (Slider, Slider), x: i32, y: i32, xrel: i32) {
    ...
    if slider_pos >= 0 && slider_pos <= 255 {
        ...
        if y < MARGIN as i32 / 2 {
            ...
            if slider_bottom.active {
                println!("bottom tap {:?}", slider_data);
    ...
    if x > (MARGIN + PADDING_LEFT + SCREEN_WIDTH + PADDING_RIGHT) as i32
        && x < (MARGIN + PADDING_LEFT + SCREEN_WIDTH + PADDING_RIGHT + MARGIN / 2) as i32
...
pub fn main() -> Result<(), i32> {
    ...
    let font_rwops = sdl2::rwops::RWops::from_bytes(FONT).unwrap();
    let font = sdl_ttf.load_font_from_rwops(font_rwops, 16).unwrap();
    ...
    let (hww_sender, hww_receiver) = mpsc::channel();
    ev.register_custom_event::<HWWDataEvent>().unwrap();
    ev.register_custom_event::<DisconnectEvent>().unwrap();
    let es = ev.event_sender();
    let new_connection_flag = Arc::new(AtomicBool::new(false));
    thread::spawn({
        let new_connection_flag = Arc::clone(&new_connection_flag);
        || network_task(new_connection_flag, evs_receiver, hww_sender)
    });
    ...
    let mut fps_formatted = String::new();
    'running: loop {
        if new_connection_flag.swap(false, Ordering::SeqCst) {
            println!("new connection");
            let (sender, receiver) = mpsc::channel();
            current_client_outbound = Some(sender);
            evs_sender.send((ev.event_sender(), receiver)).unwrap();
        }
        for event in event_pump.poll_iter() {
            match event {
                ...
                Event::User { .. } => {
                    if let Some(_) = event.as_user_event_type::<DisconnectEvent>() {
        ...
        while let Ok(data) = hww_receiver.try_recv() {
            unsafe {
                bitbox02::bindings::usb_packet_process(
                    data.as_ptr() as *const bitbox02::bindings::USB_FRAME
                )
            };
        }
        ...
        if now.duration_since(last_blit) > Duration::from_micros(16667) {
            ...
            let fps = 1000.0 / ((now - last_blit).as_millis() as f32);
            fps_formatted.clear();
            write!(fps_formatted, "{fps:.2}").unwrap();
            last_blit = now;

            let text_surf = font
                .render(&fps_formatted)
                .solid(Color::RGB(0, 0, 0))
                .unwrap();
            let text = text_surf.as_texture(&tc).unwrap();
            let fps_box = Rect::new(10, 5, text_surf.width(), text_surf.height());
            canvas
                .copy(&text, None, fps_box)
                .expect("failed to copy text to canvas");
