...
static uECC_word_t regularize_k(
    const uECC_word_t* const k,
    uECC_word_t* k0,
    uECC_word_t* k1,
    uECC_Curve curve)
{
    ...
    uECC_Curve curve)
{
    ...
    uECC_generate_k_rfc6979(secret, private_key, message_hash, hash_size, hash_context, curve);
...
#else
    uECC_vli_bytesToNative(
        s, signature + curve->num_bytes, BITS_TO_BYTES(curve->num_n_bits)); /* tmp = d */
#endif
...
void uECC_generate_private_key(
    ...
    uECC_Curve curve)
{
    ...
    uECC_vli_bytesToNative(_private_master, private_master, BITS_TO_BYTES(curve->num_n_bits));
    uECC_vli_bytesToNative(_z, z, BITS_TO_BYTES(curve->num_n_bits));
#endif

    wordcount_t num_n_words = BITS_TO_WORDS(curve->num_n_bits);
    uECC_vli_modAdd(_private_child, _private_master, _z, curve->n, num_n_words);
...
int uECC_isValid(uint8_t* private_key, uECC_Curve curve)
{
    ...
    return (
        !uECC_vli_isZero(_private, BITS_TO_WORDS(curve->num_n_bits)) &&
